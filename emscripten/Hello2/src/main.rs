...
impl RustPlugin for HelloWorld {
    fn init(&mut self, screen: Arc<Mutex<gfx::Screen>>) -> f64 {
        let data = include_bytes!("helloworld.dpx8");
        let mut buf_reader = Cursor::new(array_to_vec(data));

        match cartridge::Cartridge::parse_raw("helloworld.dpx8", &mut buf_reader, false) {
